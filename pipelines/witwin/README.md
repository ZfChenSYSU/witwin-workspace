...
此目录当前仍归属 `work/wsl-witwin` 分支，目标运行环境改为服务器，用于 WiTwin 仿真、参数反演、实验脚本和经过筛选
的可复现结果。
...
`/opt/witwin/venv/bin/python` 只属于既有 WSL 环境记录。服务器迁移完成前先按
[`服务器工作区与远程操作指南`](../../docs/reference/server/服务器工作区与远程操作指南.md)
复核目标环境，不在代码或新文档中沿用未验证的绝对路径。历史实验报告记录工作区提交、
两个源码子模块提交、依赖版本、配置和随机种子；新报告应写入
`docs/history/branches/work-wsl-witwin/experiments/`。
