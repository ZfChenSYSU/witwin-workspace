...
- 还没有把每条 CSI 正式物化到手机时间轴并关联 ARKit/IMU；
...
1. 实现并验证 CSI 时间轴物化工具，记录映射文件 SHA-256、设备组合和适用时间。
2. 完成 3 组以上静止重复 session，并在每次采集前后加入静态参考段。
3. 冻结接收端装配，建立幅度、RSSI、包密度和时钟残差基线。
4. 独立推进 CFO/SFO/PDD、公共相位和射频链响应校准；通过前以幅度为真实主线。
5. 保持手机上行与 AP 下行严格分离，不把监听网卡 CSI 表述为普通路由器端 CSI。
